...
- [x] Publish the SDK, React components and MCP server to npm (MIT) as `@arclighthq/*`; see [Publishing](publishing.md).
...
- [x] Integration guide and a runnable reference Next.js admin ([`examples/nextjs-admin`](../examples/nextjs-admin)), matching the Vera & Co. admin (Next.js on Vercel). See [Embedding](embedding.md#reference-integration-a-nextjs-admin-on-vercel).
...
- [x] Instagram Business Discovery signals (followers, activity), including capacity for businesses without a website (D9). See [Instagram signals](instagram.md).
